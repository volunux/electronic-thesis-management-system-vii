...
@VxRepository()
export abstract class VxEntityOneRepositoryImpl<T> extends AbstractBaseOrmRepositoryImpl<T> {

  protected readonly search: AbbreviationEntitySearch<T> = AbbreviationEntitySearch.getInstance({});
  ...
  public async findAll(q: EntityQueryConfig): Promise<T[]> {
    ...
    if (q !== null && q !== undefined) {
      ...
      else if (q.getParameter(`type`) === `abbreviation`) { this.search.abbreviation(qb, q); }
    ...
    return await qb.select([`vx._id`, `vx.name`, `vx.abbreviation`, `vx.updated_on`]).orderBy(`vx.updated_on`, `ASC`).addOrderBy(`vx._id`, `ASC`).limit(10).getMany();
  }

  public async updateOne(id: string | number, userId?: number): Promise<T | null> {
    let entry: T | undefined = await getRepository(this.VxEntity).createQueryBuilder(`vx`).where({ '_id': id }).select([`vx._id`, `vx.name`, `vx.abbreviation`, `vx.description`]).limit(1).getOne();
    if (entry === undefined) return null;
    return entry;
  }

  public async deleteOne(id: string | number, userId?: number): Promise<T | null> {
    let entry: T | undefined = await getRepository(this.VxEntity).createQueryBuilder(`vx`).where({ '_id': id }).select([`vx._id`, `vx.name`, `vx.abbreviation`]).limit(1).getOne();
    if (entry === undefined) return null;
    return entry;
  }
